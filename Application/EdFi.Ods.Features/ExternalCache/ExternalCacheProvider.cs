...
namespace EdFi.Ods.Features.ExternalCache
{
    public class ExternalCacheProvider : IExternalCacheProvider
    {
        ...
        void ICacheProvider.SetCachedObject(string keyName, object obj)
        {
            ...
            {
                AbsoluteExpirationRelativeToNow = _absoluteExpiration.TotalSeconds > 0 ? (TimeSpan?)_absoluteExpiration : null,
                SlidingExpiration = _slidingExpiration.TotalSeconds > 0 ? (TimeSpan?)_slidingExpiration : null
            });
        }
        ...
        void ICacheProvider.Insert(string key, object value, DateTime absoluteExpiration, TimeSpan slidingExpiration)
        {
            ...
            {
                AbsoluteExpiration = absoluteExpiration < DateTime.MaxValue ? (DateTimeOffset?)absoluteExpiration : null,
                SlidingExpiration = slidingExpiration.TotalSeconds > 0 ? (TimeSpan?)slidingExpiration : null
            });
        }
